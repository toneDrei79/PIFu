...
class Evaluator:
    ...
    def load_image(self, image_path, mask_path):
        ...
        image = transforms.Resize((self.load_size, self.load_size))(image)
        image = self.to_tensor(image)
        image = mask.expand_as(image) * image

        # print(mask.unsqueeze(0).shape, image.unsqueeze(0).shape)
        return {
            'name': img_name,
...
if __name__ == '__main__':
    ...
    test_images = glob.glob(os.path.join(opt.test_folder_path+'rgb/', '*'))
    test_masks = glob.glob(os.path.join(opt.test_folder_path+'mask/', '*'))
    test_images.sort()
    test_masks.sort()
